...
public partial class Chat
{
    ...
    {
        if (IsSendingMessage || string.IsNullOrWhiteSpace(MessageInput) || SelectedFriend == null || !ChatService.IsConnected)
            return;
        ...
        try
        {
            ...
            {
                if (ChatService.GetConnectionState() != HubConnectionState.Connected)
                {
                    MessageDeliveryStatus = "Нет соединения с сервером. Сообщение не отправлено.";
        ...
        {
            MessageDeliveryStatus = "Ошибка при отправке.";
            MessageInput = content;
            AddToast($"Не удалось отправить: {ex.Message}", "error");
        }
        finally
        {
